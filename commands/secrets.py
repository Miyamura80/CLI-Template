...
@app.command(
    ...
    epilog=examples_epilog(
        "mycli secrets set OPENAI_API_KEY sk-...",
        "echo sk-... | mycli secrets set OPENAI_API_KEY --stdin",
        "mycli --dry-run secrets set OPENAI_API_KEY sk-...",
    ),
)
def set_secret(
...
@app.command(
    "get",
    epilog=examples_epilog(
        "mycli secrets get OPENAI_API_KEY",
        "mycli secrets get OPENAI_API_KEY --reveal",
    ),
)
def get_secret(
    key: Annotated[str, typer.Argument(help="Secret key name.")],
...
@app.command(
    epilog=examples_epilog(
        "mycli secrets delete OPENAI_API_KEY",
        "mycli --dry-run secrets delete OPENAI_API_KEY",
    )
)
def delete(
...
def _load_import_values(
    ...
    if sys.stdin.isatty():
        console.print(
            ...
            "pipe a .env in, e.g. cat .env | mycli secrets import --stdin"
        )
        raise typer.Exit(code=2)
...
@app.command(
    "import",
    epilog=examples_epilog(
        "mycli secrets import --file .env",
        "cat .env | mycli secrets import --stdin",
        "mycli --dry-run secrets import --file .env",
    ),
)
def import_secrets(
    file: Annotated[
...
@app.command(
    "export",
    epilog=examples_epilog("mycli secrets export", "mycli secrets export --reveal"),
)
def export_secrets(
    reveal: Annotated[
        bool,
